...
class RepRSAB(nn.Module):
    def __init__(self, in_channels, use_atten, conv=nn.Conv2d, deploy=False):
        ...
        self.c1_r = BSConvU(in_channels, self.rc, kernel_size=3, **kwargs)
        ...
        if self.use_atten:
            self.c5 = nn.Conv2d(self.rc, in_channels, 1)
...
class LSANet(nn.Module):
    def __init__(
        ...
        self.B1 = RepRSAB(in_channels=num_feat, use_atten=False, conv=self.conv, deploy=self.deploy)
        self.B2 = RepRSAB(in_channels=num_feat, use_atten=True, conv=nn.Conv2d, deploy=self.deploy)
        self.B3 = RepRSAB(in_channels=num_feat, use_atten=False, conv=self.conv, deploy=self.deploy)
        self.B4 = RepRSAB(in_channels=num_feat, use_atten=True, conv=nn.Conv2d, deploy=self.deploy)
        self.B5 = RepRSAB(in_channels=num_feat, use_atten=False, conv=self.conv, deploy=self.deploy)
        self.B6 = RepRSAB(in_channels=num_feat, use_atten=True, conv=nn.Conv2d, deploy=self.deploy)
    ...
    def forward(self, input):
        ...
        feature_fusion = self.c2(
                    self.GELU(self.c1(torch.cat([out_B1, out_B2, out_B3, out_B4, out_B5, out_B6], dim=1)))
                )

        out = self.upsampler(feature_fusion + out_fea) + self.mean
